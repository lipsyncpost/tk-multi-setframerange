...
class SetFrameRange(Application):
    ...
    def get_versions_list_from_shotgun(self):
        ...
        for step in ["plates", "animation"]:
            self.logger.info("Getting frame range from published {}...".format(step))
    ...
    def get_frame_range_from_shotgun(self):
        ...
        if data[sg_in_field] is None or data[sg_out_field] is None:
            ...
            if version_data_list:
                ...
                raise tank.TankError(
                    "Configuration error: Shotgun data was not found and "
                    "no plates or animation was found for this entity"
                )
        else:
